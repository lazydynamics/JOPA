...
def _frozen_msg(image):
    mu, log_std = vae_pretrained.encode(image)
...
H_base = baseline.blocks[0].transition.A
...
try:
    ...
    base_preds = baseline_out["predictions"]
    e2e_preds = e2e_out["predictions"]
    means_b = np.array(baseline_out["means"])
    means_e = np.array(e2e_out["means"])
    ...
    for i, ax in enumerate(axes[2]):
        ...
        ax.imshow(np.array(base_preds[idx]).reshape(28, 28), cmap="inferno")
    ...
    for i, ax in enumerate(axes[3]):
        idx = n_observed + i * (n_predicted // 10)
        ax.imshow(np.array(e2e_preds[idx]).reshape(28, 28), cmap="inferno")
    ...
    ax_loss.plot(loss_history, "b-", linewidth=0.5)
    ax_loss.set(xlabel="gradient step", ylabel="loss", title="M-step loss")
    ax_loss.grid(True, alpha=0.3)

    ax_det = fig2.add_subplot(gs[0, 1])
    ax_det.plot(range(1, len(diag["det_history"]) + 1), diag["det_history"], "r-o", markersize=3)
    ...
    eig_mods = np.array([np.sort(np.abs(np.array(e)))[::-1] for e in diag["eig_history"]])
